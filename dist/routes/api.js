...
import { gamesTable, setupsTable, setupGamesTable, offerTable, offerDetailsTable, bookingTable, bookingAndGames, bookingAndOffersTable, slotLocksTable, bookingSlotsTable } from '../core/db/schema.js';
    ...
    try {
        ...
        if (query) {
            games = await db
                .select()
                .from(gamesTable)
                .where(and(eq(gamesTable.isActive, true), ilike(gamesTable.name, `%${query}%`)));
        }
        else {
            games = await db
                .select()
...
api.post('/bookings/review', async (c) => {
    try {
        ...
        const dateOptions = { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Asia/Kolkata' };
...
api.get('/bookings', authMiddleware, async (c) => {
    try {
        const requester = c.get('jwtPayload');
        let bookings;
...
api.patch('/bookings/:id/status', authMiddleware, requireRole(['ADMIN', 'SUPER_ADMIN']), async (c) => {
    try {
        ...
        const snapshot = updated.setupSnapshot;
...
api.get('/slots/available', async (c) => {
    try {
        ...
        const bookedIntervals = await db
            ...
            .from(bookingTable)
            .where(and(eq(bookingTable.setupId, setupId), lt(bookingTable.startTime, dayEnd), gt(bookingTable.endTime, dayStart), ne(bookingTable.status, 'CANCELLED')));
        ...
        const activeLocks = await db
            ...
            .from(slotLocksTable)
            .where(and(eq(slotLocksTable.setupId, setupId), eq(slotLocksTable.slotDate, date), gt(slotLocksTable.lockedUntil, new Date())));
    ...
    try {
        ...
        const result = await db.transaction(async (tx) => {
            ...
            const [existingBooking] = await tx
                .select()
                .from(bookingTable)
                .where(and(eq(bookingTable.setupId, setupId), lt(bookingTable.startTime, requestedEnd), gt(bookingTable.endTime, requestedStart), ne(bookingTable.status, 'CANCELLED')));
            if (existingBooking) {
            ...
            const [existingLock] = await tx
                .select()
                .from(slotLocksTable)
                .where(and(eq(slotLocksTable.setupId, setupId), lt(slotLocksTable.startTime, requestedEnd), gt(slotLocksTable.endTime, requestedStart), gt(slotLocksTable.lockedUntil, new Date()), ne(slotLocksTable.lockToken, lockToken)));
            if (existingLock) {
            ...
            await tx
                ...
                .where(and(eq(slotLocksTable.setupId, setupId), eq(slotLocksTable.lockToken, lockToken)));
            // Perform lock
